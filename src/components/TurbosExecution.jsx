...
import { Info, Minus, Plus, Calendar as CalendarIcon, ChevronUp, ChevronDown, Loader2, ChevronLeft, ChevronRight, ChevronRightIcon } from 'lucide-react';
...
const getSelectedAccountFromStorage = () => {
  try {
    ...
    console.error('[TurbosExecution] Error getting selectedAccount from localStorage:', e);
  }
  return null;
};
...
const TurbosExecution = ({ selectedMarket, tradeType = 'turbos', aiPrediction, onBarrierChange, hidePrediction = false, onTradePlaced, hideActionButtons = false, onProposalsSync }) => {
    ...
    const [barrier, setBarrier] = useState('+2.12'); // Default to first available barrier
    const [barrierError, setBarrierError] = useState(null);
    const [availableBarriers, setAvailableBarriers] = useState([]);
    const [isLoadingBarriers, setIsLoadingBarriers] = useState(false);
    ...
    // Fetch available barriers from API
    useEffect(() => {
        const fetchAvailableBarriers = async () => {
            if (!selectedMarket?.symbol || !api || !isConnected) return;
            
            setIsLoadingBarriers(true);
            try {
                // Make a proposal request with a dummy barrier to get available barriers
                const testParams = {
                    proposal: 1,
                    amount: stake,
                    basis: 'stake',
                    ...
                    barrier: '+1.00', // Dummy barrier
                    duration: durationValue,
                    duration_unit: durationUnit || 'm',
                };
                
                const response = await api.send(testParams);
                
                // Check if error contains barrier_choices
                if (response.error && response.error.details && response.error.details.barrier_choices) {
                    const barriers = response.error.details.barrier_choices;
                    setAvailableBarriers(barriers);
                    // Set default barrier to first available if not set
                    if (!barrier || barrier === '0' || !barriers.includes(barrier)) {
                        setBarrier(barriers[0] || '+2.12');
                    }
                } else if (response.proposal) {
                    // If proposal succeeds, we might need to try with different barriers
                    // For now, use common barrier values
                    const defaultBarriers = ['+2.12', '+3.73', '+6.56', '+11.55', '+20.33', '+35.77', '+62.96', '+110.81', '+195.02', '+343.25'];
                    setAvailableBarriers(defaultBarriers);
                    if (!barrier || barrier === '0') {
                        setBarrier(defaultBarriers[0]);
                    }
                }
            } catch (error) {
                // Use default barriers if fetch fails
                const defaultBarriers = ['+2.12', '+3.73', '+6.56', '+11.55', '+20.33', '+35.77', '+62.96', '+110.81', '+195.02', '+343.25'];
                setAvailableBarriers(defaultBarriers);
                if (!barrier || barrier === '0') {
                    setBarrier(defaultBarriers[0]);
                ...
                setIsLoadingBarriers(false);
            }
        };
        
        fetchAvailableBarriers();
    }, [selectedMarket?.symbol, api, isConnected, stake, durationValue, durationUnit]);
    
    // Report barrier to chart
    useEffect(() => {
        if (!onBarrierChange || !selectedMarket?.symbol || !lastTick || lastTick.symbol !== selectedMarket.symbol || !barrier || barrier === '0') {
            onBarrierChange?.([]);
            return;
        }
        ...
        // Parse barrier string (e.g., "+2.12" -> 2.12)
        const barrierValue = parseFloat(barrier.replace(/^\+/, '')) || 0;
        if (barrierValue === 0) {
            onBarrierChange?.([]);
            return;
        }
        const barrierPrice = currentPrice + barrierValue;
        onBarrierChange?.([{
            price: barrierPrice,
            label: `≡ ${barrier}`,
    ...
    const fetchProposal = async (contractType, extra = {}) => {
        ...
        // Validate barrier is selected and available
        if (!barrier || barrier === '0' || !availableBarriers.includes(barrier)) {
            if (availableBarriers.length > 0) {
                // Use first available barrier if current is invalid
                const firstBarrier = availableBarriers[0];
                setBarrier(firstBarrier);
                // Continue with first barrier
            } else {
                return null;
            }
        }
        ...
        // Barrier must be one of the available choices from the API
        const baseParams = {
            ...
            symbol: selectedMarket.symbol,
            // Use the barrier string directly (e.g., "+2.12") - must match available barriers
            barrier: barrier,
            ...extra,
        };
        ...
        try {
            ...
            if (response.error) {
                ...
                // Check if error contains barrier_choices (available barriers)
                if (response.error.details && response.error.details.barrier_choices) {
                    const barriers = response.error.details.barrier_choices;
                    setAvailableBarriers(barriers);
                    // Update barrier to first available if current is not in list
                    if (!barriers.includes(barrier)) {
                        setBarrier(barriers[0] || '+2.12');
                    }
                    setBarrierError(null);
                } else if (errorCode === 'ContractBuyValidationError' && errorMessage.includes('barrier')) {
                ...
                if (!isRateLimit && !response.error.details?.barrier_choices) {
                }
                return null;
            }
    ...
    useEffect(() => {
        // Only fetch proposals if we have available barriers and a valid barrier selected
        if (availableBarriers.length === 0 || !barrier || barrier === '0' || !availableBarriers.includes(barrier)) {
            setUpProposal(null);
    ...
    }, [stakeOrPayout, stake, payout, durationValue, durationUnit, endDate, durationOrEndtime, barrier, selectedMarket?.symbol, availableBarriers, api, isConnected]);
    ...
    const handleBarrierChange = (value) => {
        setBarrier(value);
        setBarrierError(null);
    };
    ...
    const handlePurchase = async (contractType, proposal) => {
        ...
        if (selectedAccount && selectedAccount.loginid && user?.loginid && user.loginid !== selectedAccount.loginid) {
            console.error('[TurbosExecution] CRITICAL: Account mismatch detected!', {
        ...
        try {
            ...
            if (user?.token && isAPIToken(user.token) && userOAuthToken) {
                console.error('[TurbosExecution] CRITICAL: User state still shows API token! Blocking trade to protect API owner account.');
                setIsPurchasing(false);
                toast({
                    title: 'Account Error',
            ...
            try {
                ...
                if (stored) {
                    ...
                    if (parsed?.token && isAPIToken(parsed.token) && userOAuthToken) {
                        console.error('[TurbosExecution] CRITICAL: localStorage still has API token! Blocking trade.');
                        setIsPurchasing(false);
                        toast({
                            title: 'Account Not Ready',
            ...
            console.log('[TurbosExecution] Placing trade with:', {
                selectedAccountLoginid: selectedAccount?.loginid,
                authenticatedLoginid: user?.loginid,
                balance: balance,
            ...
            if (response.buy) {
                ...
                if (onTradePlaced) {
                    onTradePlaced({
                        ...
                        buy_price: buyPrice,
                        payout: payout,
                        contract_type: contractType === 'Up' ? 'TURBOSLONG' : 'TURBOSSHORT',
    ...
    }, [upProposal, downProposal, isPurchasing, canTrade, api, isConnected, onProposalsSync, handlePurchase, toast]);
            ...
            <div className="space-y-1">
                ...
                {isLoadingBarriers ? (
                    <div className="h-10 flex items-center justify-center text-sm text-gray-500">
                        Loading available barriers...
                    </div>
                ) : availableBarriers.length > 0 ? (
                    <>
                        <Select value={barrier} onValueChange={handleBarrierChange}>
                            <SelectTrigger className={cn("w-full", barrierError && "border-red-500")}>
                ...
                <div className="border-t border-gray-200 pt-4 flex gap-2">
                    <div className="flex-1">
                        ...
                        <StyledTradeButton
                            ...
                            disabled={!upProposal || !api || !isConnected || !canTrade || isPurchasing}
                            isLoading={isPurchasing}
                    ...
                    <div className="flex-1">
                        ...
                        <StyledTradeButton
                            ...
                            disabled={!downProposal || !api || !isConnected || !canTrade || isPurchasing}
                            isLoading={isPurchasing}
                            getPayoutPct={getPayoutPct}
                            getPayoutAmount={getPayoutAmount}
                        />
